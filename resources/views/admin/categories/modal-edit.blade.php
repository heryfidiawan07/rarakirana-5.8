<a class="btn btn-warning btn-sm" href="#" data-toggle="modal" data-target=".edit-category-{{$parent->id}}">
    <i class="fas fa-edit"></i>
</a>

<div class="modal fade edit-category-{{$parent->id}}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit Category {{$parent->name}} ?</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form method="POST" action="/admin/category/{{$parent->id}}/update">
                    {{csrf_field()}}
                    ...
                    <div class="form-group">
                        <label for="icon">
                            ...
                            @endif
                            <small>Default <i class="fas fa-square-full"></i></small>
                        </label>
                    ...
                    <div class="form-group">
                        <label for="parent_edit">Category Parent</label>
                        <select class="custom-select" name="parent_edit">
                            ...
                            @else
                                @if ($parent->childs->count()==0)
                                    <option value="0" class="from-control">Default</option>
                                @endif
                                @foreach ($categories->where('parent_id',0)->where('id','<>',$parent->id) as $category)
