          ...
          <tr>
            <td style="{{ $fontFamily }} {{ $style['email-body_cell'] }}">
              ...
              <p style="{{ $style['paragraph'] }}">
                Pesanan baru dari {{$order->user->name}} dengan No Invoice {{$order->payment->no_invoice}}
              </p>
          <!-- Action Button -->
              <table style="{{ $style['body_action'] }}" align="center" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    ...
                    ?>
                    <a href="{{ config('app.url') }}/admin/order/{{$order->slug_token}}/details"
                      style="{{ $fontFamily }} {{ $style['button'] }} {{ $style[$actionColor] }}"
                      class="button"
                      target="_blank">
                      Klik Untuk Cek Order !
                    </a>
                  </td>
                </tr>
              </table>
              ...
              <table style="{{ $style['body_sub'] }}">
                <tr>
                  <td style="{{ $fontFamily }}">
                    ...
                    <p style="{{ $style['paragraph-sub'] }}">
                      <a style="{{ $style['anchor'] }}" href="{{ config('app.url') }}/admin/order/{{$order->slug_token}}/details" target="_blank">
                        {{ config('app.url') }}/admin/order/{{$order->slug_token}}/details
                      </a>
                    </p>
                  </td>
                </tr>
