          ...
          <tr>
            <td style="{{ $fontFamily }} {{ $style['email-body_cell'] }}">
              ...
              <p style="{{ $style['paragraph'] }}">
                Pesanan kamu dengan No Invoice {{$order->payment->no_invoice}} sedang dalam pengiriman oleh kurir, dengan No Resi {{$order->resi_kurir}}
              </p>
          <!-- Action Button -->
              <table style="{{ $style['body_action'] }}" align="center" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
                    ...
                    ?>
                    <a href="{{ config('app.url') }}/order/{{$order->user->slug}}/details/{{$order->slug_token}}"
                      style="{{ $fontFamily }} {{ $style['button'] }} {{ $style[$actionColor] }}"
                      class="button"
                      target="_blank">
                      Lihat detail pesanan kamu disini !
                    </a>
                  </td>
                </tr>
              </table>
              ...
              <table style="{{ $style['body_sub'] }}">
                <tr>
                  <td style="{{ $fontFamily }}">
                    ...
                    <p style="{{ $style['paragraph-sub'] }}">
                      <a style="{{ $style['anchor'] }}" href="{{ config('app.url') }}/order/{{$order->user->slug}}/details/{{$order->slug_token}}" target="_blank">
                        {{ config('app.url') }}/order/{{$order->user->slug}}/details/{{$order->slug_token}}
                      </a>
                    </p>
                  </td>
                </tr>
