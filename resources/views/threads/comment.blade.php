@if ($thread->comments->count())
    @foreach ($thread->comments->where('parent_id',0) as $comment)
                    ...
                    <p>
                        ...
                        @auth
                            @if (Auth::user()->id === $child->user->id)
                                | <small><a data-toggle="collapse" href="#update_{{$child->id}}" role="button" aria-expanded="false" aria-controls="collapseExample">Sunting</a></small>
                            @endif
                        @endif
                    </p>
                    @auth
...
@auth <form method="POST" action="/thread/{{$thread->slug}}/comment/store"> @endif
    @auth @csrf @endif
    <div class="form-group">
